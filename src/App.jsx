...
import { RevealEffect } from "./components/effects/RevealEffect";
import { PowerDeltaLabel } from "./components/effects/PowerDeltaLabel";
...
function CardPopup({ card, onDismiss, playable = true, onUnplay = null }) {
  return (
    <>
      <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(5px)",WebkitBackdropFilter:"blur(5px)"}} onClick={onDismiss} />
      <div style={{position:"fixed",left:"50%",bottom:150,transform:"translateX(-50%)",zIndex:201,display:"flex",flexDirection:"column",alignItems:"center",gap:12,animation:"popup-rise 0.22s cubic-bezier(.22,1,.36,1)",width:"min(240px,64vw)"}} onClick={e => e.stopPropagation()}>
        ...
        <div style={{display:"flex",gap:8,width:"100%"}}>
          ...
          {onUnplay && <button className="btn btn-danger" style={{flex:1,padding:"10px"}} onClick={() => { onUnplay(); onDismiss(); }}>↩ Return</button>}
        </div>
      </div>
    </>
  );
...
function GameScreen({ cards, deckCards, onBack }) {
    ...
    queue.forEach((play) => {
      timeouts.push(setTimeout(() => {
        ...
        setGame(prev => {
          ...
          const card = play.isPlayer ? zone.pCards.find(c => c.id === play.cardId) : zone.aCards.find(c => c.id === play.cardId);
          if (!card) return prev;
          ...
          setTimeout(() => setRevealFx({ cardId: play.cardId, snapName: card.snapName || "", zoneIdx: play.zoneIdx, isPlayer: play.isPlayer }), 220);
      ...
      setGame(prev => {
        ...
        if (s.turn >= TURNS) {
          if (s._captainMarvelCardId) {
            ...
            if (cmZone >= 0) {
              const scores = s.zones.map((z, zi) => ({ zi, pp: zonePower(z.pCards, s, true, zi) + klawBonusForZone(s, true, zi), ap: zonePower(z.aCards, s, false, zi) + klawBonusForZone(s, false, zi) }));
  ...
  const startDrag = (e, cardId, source = "hand") => {
    ...
    const onMove = ev => {
      ...
      if (!moved && (Math.abs(x - startX) > 15 || Math.abs(y - startY) > 15)) moved = true;
      dragRef.current = { cardId, x, y };
      setDrag({ cardId, x, y });
      if (moved) { const zi = hitZone(x, y); setDragOver(zi); }
    };
    ...
    const onEnd = ev => {
      ...
      if (!moved) {
        ...
        if (zi !== null) {
          const card = game.playerHand.find(c => c.id === cardId);
          if (!card) return;
          if (game.zones[zi].pCards.length >= MAX_PER_SIDE) return;
          const cost = card.energy ?? card.clout;
          const spent = (game.playerPlaysThisTurn || []).reduce((s, p) => s + p.energy, 0);
          if (cost <= game.turn - spent && inPlay) placeCard(cardId, zi);
        }
      } else if (source === "zone") {
        if (!inPlay) return;
...
// ─── Deck helpers ─────────────────────────────────────────────────────────────
function loadDecks() { try { return JSON.parse(localStorage.getItem(DECKS_KEY) || "null") || []; } catch { return []; } }
function saveDecks(decks) { try { localStorage.setItem(DECKS_KEY, JSON.stringify(decks)); } catch {} }
function makeRandomDeck(allCards) { return shuffle([...allCards]).slice(0, 12).map(c => c.id); }
...
function DeckBuilderScreen({ deck, allCards, onSave, onBack }) {
  ...
  const toggle = id => setCardIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : prev.length >= 12 ? prev : [...prev, id]);
  const handleSave = () => onSave({ ...deck, name: name.trim() || "Deck", cardIds, id: deck?.id || uid() });
  return (
    <div className="screen">
      <div className="screen-header">
        <button className="btn btn-ghost" onClick={onBack}>← Back</button>
        <input value={name} onChange={e => setName(e.target.value)} style={{flex:1,background:"transparent",border:"none",color:"var(--text)",fontSize:"1.1rem",fontWeight:700,outline:"none",textAlign:"center"}} maxLength={20} />
        <button className="btn btn-primary" onClick={handleSave}>Save</button>
      </div>
          ...
          return (
            <div key={card.id} className="deck-builder-slot" onClick={() => toggle(card.id)}>
              <div className={`coll-card${inDeck?" in-deck":""}`} style={{"--ch":hue}}>
                {card.imageUrl ? <img className="coll-card-art" src={card.imageUrl} alt={card.name} /> : <div className="coll-card-emoji">{emojiOf(hue)}</div>}
                <div className="coll-card-overlay"/>
...
const DECK_ICONS    = ["⚔️","🔮","💀","🌀","🎭","🦅"];
const DEFAULT_DECKS = [{ id:"deck1", name:"Deck 1", cardIds:[] }, { id:"deck2", name:"Deck 2", cardIds:[] }, { id:"deck3", name:"Deck 3", cardIds:[] }];

function DeckSelectScreen({ allCards, onPlay, onBack }) {
  const [decks,    setDecks]   = useState(() => { const saved = loadDecks(); return DEFAULT_DECKS.map(d => saved.find(s => s.id === d.id) || d); });
  const [selected, setSelected] = useState(null);
  const [building, setBuilding] = useState(null);
  const persistAndSet = updated => { setDecks(updated); saveDecks(updated); };
  const handleSaveDeck = deck => { persistAndSet(decks.map(d => d.id === deck.id ? deck : d)); setBuilding(null); };
  const resolveCards = (cardIds, allCards) => {
    let picked = cardIds.map(id => allCards.find(c => c.id === id)).filter(Boolean);
    if (picked.length < 12) { const used = new Set(picked.map(c => c.id)); const pool = shuffle(allCards.filter(c => !used.has(c.id))); while (picked.length < 12 && pool.length) picked.push(pool.shift()); }
    return shuffle(picked.slice(0, 12));
  };
  if (building) return <DeckBuilderScreen deck={building} allCards={allCards} onSave={handleSaveDeck} onBack={() => setBuilding(null)} />;
  return (
    <div className="screen deck-select-screen">
      ...
      <div className="deck-list">
        <div className="deck-card" onClick={() => onPlay(shuffle([...allCards]).slice(0,12))} style={{background:"linear-gradient(135deg,rgba(112,64,240,0.2),rgba(160,112,255,0.1))"}}>
          ...
          <div className="deck-card-info"><div className="deck-card-name">Random Deck</div><div className="deck-card-sub">12 random cards — pure chaos</div></div>
        ...
        {decks.map((deck, i) => {
          ...
          return (
            <div key={deck.id} className={`deck-card${isSelected?" deck-selected":""}`} onClick={() => setSelected(p => p === deck.id ? null : deck.id)}>
              <div className="deck-card-icon">{DECK_ICONS[i]}</div>
              <div className="deck-card-info">
                <div className="deck-card-name">{deck.name}</div>
                <div className="deck-card-sub">{deckCards.length > 0 ? `${deckCards.length} cards · avg ${(deckCards.reduce((s,c)=>s+(c.energy??c.clout),0)/deckCards.length).toFixed(1)} energy` : "Empty — tap Edit to build"}</div>
              </div>
      ...
      {selected && (
        <div className="deck-play-bar">
          <button className="btn btn-primary" style={{width:"100%",padding:"14px"}} onClick={() => { const deck = decks.find(d=>d.id===selected); onPlay(resolveCards(deck?.cardIds||[], allCards)); }}>
...
const LOADING_LINES = ["Initializing interdimensional portals...","Bribing Detective Scrotum...","Inflating Buttermilk Androgyna...","Calibrating Mr. Plaigan's drain...","Waking Sleepy Butterson...","Counting Bonald Brum's brain cells...","Negotiating with Goblin Addict...","Locating Souvenir Cheeseburger...","Loading ZANY card data...","Preparing interdimensional battlefield...","Shuffling 77 degenerates...","Almost ready..."];
  ...
  return (
    <div style={{height:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:32,background:"radial-gradient(ellipse at 50% 55%, #16082e 0%, #05050f 68%)",padding:"0 40px"}}>
      ...
      <div style={{width:"100%",maxWidth:280,display:"flex",flexDirection:"column",gap:10}}>
        ...
        <div style={{fontFamily:"var(--f-mono,monospace)",fontSize:"0.62rem",color:"rgba(255,255,255,0.4)",letterSpacing:"0.05em",textAlign:"center",minHeight:"1.2em"}}>{LOADING_LINES[lineIdx] || "Loading..."}</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [cards, setCards] = useState(() => {
    try {
      ...
      const merged = SPREADSHEET_CARDS.map(c => ({ ...c, imageUrl: userImgs[c.id] || CARD_IMAGES[c.id] || c.imageUrl || null }));
      const baseIds = new Set(SPREADSHEET_CARDS.map(c => c.id));
      Object.values(userExtra).forEach(c => { if (!baseIds.has(c.id)) merged.push(c); });
      return merged;
    } catch { return SPREADSHEET_CARDS; }
  });
